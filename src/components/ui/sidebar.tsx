...
const SIDEBAR_COOKIE_NAME = 'default-sidebar'
...
>(
  (
    ...
    // Initialize from cookie on client side only
    React.useEffect(() => {
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'bg-sidebar fixed inset-y-0 z-30 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex',
            side === 'left'
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-2 overflow-auto border-t border-b group-data-[collapsible=icon]:overflow-hidden',
        className
      )}
      {...props}
    />
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-sm font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
  'cursor-pointer peer/menu-button flex w-full items-center gap-1 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground group-data-[collapsible=icon]:p-1! [&>span:last-child]:truncate [&>svg]:shrink-0',
  {
    variants: {
      variant: {
        default: '',
        outline:
          'bg-background shadow-[0_0_0_1px_hsl(var(--sidebar-border))] group-hover/menu-item:shadow-[0_0_0_1px_hsl(var(--sidebar-accent))]',
      },
      size: {
        default: 'h-8 text-sm',
        sm: 'h-7 text-xs',
...
>(
  (
    { dataSidebar, className, asChild = false, showOnHover = false, ...props },
    ref
  ) => {
    const Comp = asChild ? Slot : 'button'
