...
}> = ({ message = 'Test announcement', politeness = 'POLITE' }) => {
...
describe('useLiveAnnouncer hook', () => {
  ...
  it('provides announceWithDelay function', () => {
    const DelayedComponent: React.FC = () => {
      ...
      return (
        <button
          onClick={() => announceWithDelay('Delayed message', 'POLITE', 100)}
        >
          Announce Delayed
        </button>
      )
...
describe('Announce Component', () => {
  ...
  it('renders custom component for display', () => {
    ...
    // The message also appears in the hidden live region, so scope the
    // query to the visible element with the .announce class.
    const announcement = screen.getByText('Visible text', {
      selector: 'span.announce',
    })
  ...
  it('applies aria-live attribute', () => {
    ...
    const announcement = screen.getByText('Test', { selector: '.announce' })
...
describe('LoadingAnnouncer Component', () => {
  it('announces loading state', () => {
    const { rerender } = render(
      <LiveAnnouncerProvider>
        <LoadingAnnouncer isLoading={false} loadingMessage="Fetching data..." />
      </LiveAnnouncerProvider>
    )

    // LoadingAnnouncer only announces on state changes, not on initial mount
    rerender(
      <LiveAnnouncerProvider>
        <LoadingAnnouncer isLoading={true} loadingMessage="Fetching data..." />
      </LiveAnnouncerProvider>
    )
