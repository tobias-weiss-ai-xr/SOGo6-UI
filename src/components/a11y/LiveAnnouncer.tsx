...
export interface LiveAnnouncerContextType {
   ...
   */
  announceWithDelay: (message: string, politeness?: PolitenessLevel, delay?: number) => void;
...
export const LiveAnnouncerProvider: React.FC<LiveAnnouncerProviderProps> = ({
  ...
  const announce = (message: string, politeness: PolitenessLevel = defaultPoliteness) => {
    setQueue(prev => [...prev, { message, politeness }]);
  };

  const announceWithDelay = (message: string, politeness: PolitenessLevel = defaultPoliteness, delay: number = 0) => {
  ...
  return (
    <LiveAnnouncerContext.Provider value={contextValue}>
      ...
      >
        {currentAnnouncement?.politeness === 'POLITE' ? currentAnnouncement.message : ''}
      </div>
      ...
      >
        {currentAnnouncement?.politeness === 'ASSERTIVE' ? currentAnnouncement.message : ''}
      </div>
...
export const LoadingAnnouncer: React.FC<LoadingAnnouncerProps> = ({
  ...
  useEffect(() => {
    if (prevLoading !== isLoading || prevError !== hasError || prevSuccess !== hasSuccess) {
      if (isLoading) {
  ...
  }, [isLoading, hasError, hasSuccess, announce, loadingMessage, successMessage, errorMessage]);
  ...
  useEffect(() => {
    notifications.forEach(notification => {
      if (!announcedIds.has(notification.id)) {
        ...
        const politeness: PolitenessLevel = notification.type === 'error' ? 'ASSERTIVE' : 'POLITE';
...
export function useAnnounce() {
  ...
  return {
     ...
     */
    announceFormState: (state: 'loading' | 'success' | 'error', formName?: string) => {
      const messages = {
        loading: formName ? `Submitting ${formName}...` : 'Submitting...',
        success: formName ? `${formName} submitted successfully` : 'Form submitted successfully',
        error: formName ? `Error submitting ${formName}` : 'Error submitting form',
      };
      const politeness: PolitenessLevel = state === 'error' ? 'ASSERTIVE' : 'POLITE';
     ...
     */
    announceToast: (message: string, type: 'success' | 'error' | 'warning' | 'info' = 'info') => {
      const politeness: PolitenessLevel = type === 'error' ? 'ASSERTIVE' : 'POLITE';
      announce(message, politeness);
    },
  };
}
